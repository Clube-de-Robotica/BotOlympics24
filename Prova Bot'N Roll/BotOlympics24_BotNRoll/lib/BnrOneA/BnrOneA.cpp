...
{   pinMode(3,OUTPUT);
    ...
    delayMicroseconds(200);//Minimo � 8
...
int BnrOneA::readLinePic()
{
	static int prevLine=0;
	int line=spiRequestWord(COMMAND_LINE_READ);
...
/***********************************************************************************************/
void BnrOneA::sendCalibrate(int valMax[],int valMin[],int vtrans)
{   
    int i,a;
    byte buffer[36]; // 35
    buffer[0]=KEY1;
	...
	for(int i=0;i<8;i++)
	{
		Serial.print(valMin[i]);
		Serial.print(" ");
	}
	Serial.println(vtrans);
	
    for(i=0;i<8;i++){
        buffer[i+2]=highByte(valMax[i]);
		buffer[i+10]=lowByte(valMax[i]);
		buffer[i+18]=highByte(valMin[i]);
		buffer[i+26]=lowByte(valMin[i]);		
    }
	buffer[34]=highByte(vtrans);
	buffer[35]=lowByte(vtrans);	
    spiSendData(COMMAND_LINE_CALIB,buffer,sizeof(buffer));
    delay(200);//Wait while command is processed (EEPROM write on PIC)
}
